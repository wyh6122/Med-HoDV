...
class BERT_LSTM_CRF(nn.Module):
    ...
    def get_output_score(self, sentence, attention_mask=None):
        ...
        embed = self.embed_drop(embeds[0])
    ...
    def forward(self, sentence, masks):
        # if sentence.shape[-1] != masks.shape[-1]:
        #     sentence[]
        lstm_feats = self.get_output_score(sentence)            # fixme
        self.lstm_feats = torch.mean(lstm_feats * masks.unsqueeze(-1).repeat([1,1,lstm_feats.shape[-1]]).byte(), dim=1)
